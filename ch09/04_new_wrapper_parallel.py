...
class BatchGenerator:
    ...
    def pop_rewards_steps(self) -> List[Tuple[float, int]]: # reward, step?
        res = list(self._rewards_steps)
        self._rewards_steps.clear()
        return res
    
    def __iter__(self): # make like a dataset
        while True:
            while exp_queue.qsize()>0:
                exp = exp_queue.get() # the first one
                if isinstance(exp, EpisodeEnded):
                    ...
                    self.fps_handler.step()# manual step
...
if __name__ == "__main__":
    ...
    play_proc = mp.Process(target=play_func, args=(params, net, args.cuda, exp_queue)) # pass in arguments like this
    play_proc.start()
