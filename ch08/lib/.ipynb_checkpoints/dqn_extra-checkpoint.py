...
class NoisyFactorizedLinear(nn.Linear):
    def __init__(self, in_features, out_features, sigma_zero = 0.4, bias=True):
        ...
        w = torch.full((out_fetures, in_features), sigma_init)
        self.sigma_weight = nn.Parameter(w)
    ...
    def forward(self,  input):
        ...
        noise_v = torch.mul(eps_in, eps_out) # apply cross product
...
class NoisyDQN(nn.Module):
    ...
    def forward(self, x):
        fx = x.float() / 256 # why?
        conv_out = self.conv(fx).view(fx.size()[0], -1) # flatten
...
class PrioReplayBuffer:
    def __init__(self, exp_source, buf_size, prob_alpha=0.6):
        self.exp_source_iter = iter(exp_source) # can get experience by calling next
    ...
    def populate(self, count):
        max_prio = self.priorities.max() if self.buffer else 1.0 # put priority to new samples
        for _ in range(count):
            ...
            self.pos = (self.pos+1) % self.capacity # self.pos always points to the idx where new sample is added
    ...
    def sample(self, batch_size):
        ...
        probs /= probs.sum()
        indices = np.random.choice(len(self.buffer), batch_size ,p=probs) # p=probs
        samples = [self.buffer[idx] for idx in indices]
        total = len(self.buffer)
        weights = (total * probs[indices]) ** (-self.beta)
        weights /= weights.max()
        
        return samples, indices, np.array(weights, dtype=np.float32) # use the weights to calculate gradients differently?
...
class DuelingDQN(nn.Module):
    def __init__(self, input_shape, n_actions):
        ...
        self.fc_adv = nn.Sequential( # advantage
    ...
    def adv_val(self, x):
        fx = x.float() / 256
        conv_out = self.conv(fx).view(fx.size()[0], -1) # batch_size, -1
...
class RainbowDQN(nn.Module): # a combination of dueling and noisy nets
    def __init__(self, input_shape, n_actions):
    ...
    def forward(self, x):
        adv, val = self.adv_val(x)
        return val + (adv - adv.mean(dim=1, keepdim=True)) # must be 2-dim to subtrace
