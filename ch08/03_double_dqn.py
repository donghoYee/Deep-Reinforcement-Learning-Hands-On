...
def calc_loss_double_dqn(batch, net, tgt_net, gamma, device="cpu", double=True):
    ...
    state_action_vals = net(states_v).gather(1, actions_v) # Q(s, a)
    state_action_vals = state_action_vals.squeeze(-1)
    
    with torch.no_grad():
        next_states_v = torch.tensor(next_states).to(device)
        if double:
            next_state_acts = net(next_states_v).max(1)[1] # get action of s' from net
            next_state_acts = next_state_acts.unsqueeze(-1)
            next_state_vals = tgt_net(next_states_v).gather(1, next_state_acts).squeeze(-1)
        else:
            next_state_vals = tgt_net(next_states_v).max(1)[0] # this is the original
    ...
    def process_batch(engine, batch):
        ...
        if engine.state.iteration % EVAL_EVERY_FRAME == 0:
            ...
            if eval_states is None:
                ...
                eval_states = [np.array(transition.state, copy=False) for transition in eval_states] # eval_states is a buffer containing history
                eval_states = np.array(eval_states, copy=False)
                engine.state.eval_states = eval_states # put in value
